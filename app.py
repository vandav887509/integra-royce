...
app = Flask(__name__)
app.secret_key = 'integra-bond-secret-2024'
...
def load_data():
    ...
    df['Grade Code'] = pd.to_numeric(df[COL_GRADE], errors='coerce')
    df['Machine']  = df[COL_MACHINE].apply(normalize_machine)
    df['Bond Type'] = df[COL_BOND_TYPE].apply(normalize_bond_type)
    ...
    for bond_type in BOND_TYPES:
        ...
        if not subset.empty:
            fig.add_trace(go.Scatter(
                x=subset['Date'], y=subset['Grade Code'],
            ...
            fig.add_hline(
                y=LOWER_LIMIT, line_dash='dash', line_color='red', line_width=2,
                annotation_text=f'  LCL={LOWER_LIMIT}',
                annotation_position='left', annotation_font_color='red'
            )
        fig.update_layout(
